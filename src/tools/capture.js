...
export function registerCaptureTools(server) {
  server.tool('capture_screenshot', 'Take a screenshot of the TradingView chart', {
    ...
    output_dir: z.string().optional().describe('Directory to save PNG (default: project screenshots/)'),
  }, async ({ region, filename, method, output_dir }) => {
    try { return jsonResult(await core.captureScreenshot({ region, filename, method, outputDir: output_dir })); }
